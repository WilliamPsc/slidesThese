        ...
        \only<1>{
            \begin{block}{Threats}
                \begin{itemize}
                    \setbeamertemplate{itemize items}[square]
                    \justifying
                    \item Software threats: malwares~\cite{FIMI-23-access}, memory overflow attackss~\cite{CWCBW-00-discex}, SQL injection, etc
                    \item Network threats: Man-In-The-Middle~\cite{CDL-16-commsurtuto}, jamming~\cite{PZ-22-commsurtuto}, DDoS, etc
                    \item Hardware threats: physical attacks such as reverse engineering, Side-Channel Attacks (SCA)~\cite{DM-21-appiot}, Fault Injection Attacks (FIA)~\cite{BCNTW-06-procieee}
                \end{itemize}
            \end{block}
        }
        \only<2>{
            \begin{block}{Threats}
                \begin{itemize}
                    \setbeamertemplate{itemize items}[square]
                    \justifying
                    \item \textbf{\textcolor{red}{Software threats}}: malwares~\cite{FIMI-23-access}, memory overflow attackss~\cite{CWCBW-00-discex}, SQL injection, etc
                    \item Network threats: Man-In-The-Middle~\cite{CDL-16-commsurtuto}, jamming~\cite{PZ-22-commsurtuto}, DDoS, etc
        ...
        \only<2>{
            \begin{exampleblock}{}
                \begin{itemize}
                    \setbeamertemplate{itemize items}[square]
                    \justifying
                    \item Avantage: no internal hardware modification to the main core.
                \end{itemize}
            \end{exampleblock}
            ...
            \begin{alertblock}{}
                \begin{itemize}
                    \setbeamertemplate{itemize items}[square]
                    \justifying
                    \item Disadvantage: needs support from the OS for the synchronisation between data and tags.
                \end{itemize}
            \end{alertblock}
        }
        \only<3>{
            \begin{exampleblock}{}
                \begin{itemize}
                    \setbeamertemplate{itemize items}[square]
                    \justifying
                    \item Avantage: hardware does not need to know DIFT tags and policies and no synchronisation is needed.
                \end{itemize}
            \end{exampleblock}
        ...
        \only<4>{
            \begin{exampleblock}{}
                \begin{itemize}
                    \setbeamertemplate{itemize items}[square]
                    \justifying
                    \item Avantage: No multicore CPU and no synchronisation are needed. Very low performances overhead.
                \end{itemize}
            \end{exampleblock}
...
\begin{frame}{Motivations}
    \begin{block}{}
        \begin{itemize}
            ...
            \item Manipulation of the power supply to control the program counter\footnote[frame]{\tiny\fullcite{TSW-16-fdtc}},
            \item EM Fault Injection (EMFI) : recover an AES key by targeting the cache hierarchy and the MMU\footnote[frame]{\tiny\fullcite{TBELB-21-jce}},
            \item Laser Fault Injection (LFI) can allow the replay of instructions\footnote[frame]{\tiny\fullcite{KDD-21-dsd}}.
        \end{itemize}
    \end{block}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
